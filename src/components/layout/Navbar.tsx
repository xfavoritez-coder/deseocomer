  ...
  return (
    <>
      ...
      <nav className={`dc-nav${isHome ? (scrolled ? " dc-nav--solid" : " dc-nav--transparent") : ""}`}>
        <Link href="/" className="dc-nav-logo" onClick={(e) => { if (pathname === "/") { e.preventDefault(); window.scrollTo({ top: 0, behavior: "smooth" }); } }}>🏮 DeseoComer</Link>
                    ...
                    <div style={{ position: "absolute", top: "100%", right: 0, width: "min(300px, 85vw)", maxHeight: 320, overflowY: "auto", background: "rgba(10,8,18,0.98)", border: "1px solid rgba(232,168,76,0.25)", borderRadius: 12, boxShadow: "0 8px 32px rgba(0,0,0,0.5)", zIndex: 1000 }}>
                      ...
                      {notifs.length === 0 ? (
                        ...
                        n.esVerificacion ? (
                          ...
                          <div key={n.id} onClick={() => { const slug = (n as any).datos?.concursoSlug; if (slug) window.location.href = `/concursos/${slug}`; setShowNotifs(false); }} style={{ padding: "10px 14px", borderBottom: "1px solid rgba(232,168,76,0.06)", cursor: "pointer", background: n.leida ? "transparent" : "rgba(232,168,76,0.04)" }}>
