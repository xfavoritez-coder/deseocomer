...
export default function AdminGenie() {
  ...
  if (loading) return <p style={{ fontFamily: "Georgia", color: "rgba(240,234,214,0.4)", padding: 40 }}>Cargando...</p>;
  if (!data) return <p style={{ fontFamily: "Georgia", color: "#ff6b6b", padding: 40 }}>Error al cargar</p>;
  ...
  return (
    <div style={{ maxWidth: 800 }}>
      <h1 style={{ fontFamily: "Georgia", fontSize: "1.4rem", color: "#e8a84c", marginBottom: 20 }}>🧞 Sesiones del Genio</h1>

      {/* Stats */}
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 24 }}>
        {[
          { label: "Interacciones", value: stats.totalInteractions, color: "#e8a84c" },
          ...
          <div key={s.label} style={{ background: "rgba(45,26,8,0.7)", border: "1px solid rgba(232,168,76,0.12)", borderRadius: 12, padding: "12px 18px", flex: "1 1 0", minWidth: 100, textAlign: "center" }}>
            <p style={{ fontFamily: "Georgia", fontSize: "1.3rem", color: s.color, margin: "0 0 2px", fontWeight: 700 }}>{s.value}</p>
            <p style={{ fontFamily: "Georgia", fontSize: "0.72rem", color: "rgba(240,234,214,0.4)", margin: 0 }}>{s.label}</p>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 20 }}>
        {Object.entries(ACTION_EMOJI).map(([action, emoji]) => (
          <span key={action} style={{ padding: "4px 10px", borderRadius: 8, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", fontFamily: "Georgia", fontSize: "0.75rem", color: "rgba(240,234,214,0.5)" }}>
      ...
      {/* Sessions list */}
      {sessions.length === 0 ? (
        <p style={{ fontFamily: "Georgia", color: "rgba(240,234,214,0.3)", textAlign: "center", padding: 40 }}>Sin sesiones aún</p>
      ) : (
          ...
          return (
            <div key={s.sessionId} style={{ background: "rgba(45,26,8,0.7)", border: "1px solid rgba(232,168,76,0.1)", borderRadius: 12, marginBottom: 8, overflow: "hidden" }}>
              ...
              <div onClick={() => setExpanded(isOpen ? null : s.sessionId)} style={{ padding: "12px 16px", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  <span style={{ fontFamily: "Georgia", fontSize: "0.85rem", color: "#f5d080" }}>
                    {s.userId ? "👤" : "👻"} {s.sessionId.slice(0, 8)}
                  </span>
                  <span style={{ fontFamily: "Georgia", fontSize: "0.72rem", color: "rgba(240,234,214,0.3)", marginLeft: 10 }}>
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <span style={{ fontFamily: "Georgia", fontSize: "0.7rem", color: "rgba(240,234,214,0.35)" }}>👁️{viewed} ✅{selected} ❌{ignored}</span>
                  ...
                  <div style={{ marginTop: 10 }}>
                    {s.actions.map((a: any, i: number) => (
                      <div key={a.id} style={{ display: "flex", gap: 8, alignItems: "center", padding: "4px 0", borderBottom: i < s.actions.length - 1 ? "1px solid rgba(255,255,255,0.03)" : "none" }}>
                        ...
                        <span style={{ fontFamily: "Georgia", fontSize: "0.78rem", color: "#f0ead6", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{a.menuItem.nombre}</span>
                        <span style={{ fontFamily: "Georgia", fontSize: "0.68rem", color: "rgba(240,234,214,0.25)" }}>{a.menuItem.local.nombre}</span>
                        <span style={{ fontFamily: "Georgia", fontSize: "0.65rem", color: "rgba(240,234,214,0.2)" }}>
...
const chipStyle: React.CSSProperties = { padding: "3px 8px", borderRadius: 8, background: "rgba(232,168,76,0.08)", border: "1px solid rgba(232,168,76,0.15)", fontFamily: "Georgia", fontSize: "0.68rem", color: "#e8a84c" };
