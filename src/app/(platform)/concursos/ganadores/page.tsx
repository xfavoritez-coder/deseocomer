...
export default function GanadoresPage() {
  const totalParticipantes = CONCURSOS_FINALIZADOS.reduce((s, c) => s + c.participantes, 0);
  const locales = new Set(CONCURSOS_FINALIZADOS.map(c => c.local)).size;
  const top3 = CONCURSOS_FINALIZADOS.slice(0, 3);
