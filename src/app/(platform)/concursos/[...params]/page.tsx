...
function ConcursoDetallePage() {
  ...
  { icon: "💛", pts: "+1", label: "Al recibir apoyo", id: "tt6", ttTitle: "Al recibir apoyo", ttText: "Cuando otro participante toca el corazón en tu perfil del ranking, tú ganas +1 punto. Solo puedes recibir un apoyo por persona cada 24 horas.", ttPts: "+1 punto para ti" },
      ...
      {/* Panel post-participación */}
      {recienUnido && refLink && (
        <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 500, background: "linear-gradient(to top, rgba(10,8,18,0.99), rgba(10,8,18,0.95))", border: "1px solid rgba(232,168,76,0.25)", borderRadius: "20px 20px 0 0", padding: "20px 20px 28px", animation: "dc-slideUp 0.4s ease", maxWidth: 500, margin: "0 auto", boxShadow: "0 -8px 40px rgba(0,0,0,0.6)" }}>
          <button onClick={() => setRecienUnido(false)} style={{ position: "absolute", top: 10, right: 14, background: "none", border: "none", color: "rgba(240,234,214,0.3)", fontSize: 20, cursor: "pointer" }}>×</button>
          <p style={{ fontSize: 28, textAlign: "center", marginBottom: 4 }}>🎉</p>
          <h3 style={{ fontFamily: "var(--font-cinzel)", fontSize: 16, fontWeight: 700, color: "#f5d080", textAlign: "center", textTransform: "uppercase", marginBottom: 4 }}>¡Ya estás participando!</h3>
          <p style={{ fontFamily: "var(--font-lato)", fontSize: 13, color: "rgba(240,234,214,0.5)", textAlign: "center", lineHeight: 1.5, marginBottom: 16 }}>Ahora invita amigos para subir en el ranking. Cada amigo que entre por tu link <strong style={{ color: "#e8a84c" }}>te da +2 o +3 puntos</strong>.</p>
          <button onClick={() => { window.open(`https://wa.me/?text=${encodeURIComponent(`Oye, estoy participando para ganar ${c?.premio ?? "un premio"} en ${c?.local ?? "DeseoComer"}. Si entras por mi link me ayudas a ganar puntos — es gratis 👉 ${refLink}`)}`, "_blank"); }} style={{ width: "100%", padding: 14, background: "#25d366", border: "none", borderRadius: 12, fontFamily: "var(--font-cinzel)", fontSize: 14, fontWeight: 700, color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 8, letterSpacing: "0.04em", textTransform: "uppercase" }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="#fff"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z" /><path d="M12 0C5.373 0 0 5.373 0 12c0 2.625.846 5.059 2.284 7.034L.789 23.492l4.63-1.476A11.93 11.93 0 0 0 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.75c-2.15 0-4.136-.683-5.762-1.843l-.413-.265-2.748.877.87-2.686-.287-.438A9.71 9.71 0 0 1 2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75z" /></svg>
            Compartir por WhatsApp
          </button>
          <button onClick={copyLink} style={{ width: "100%", padding: 11, background: "transparent", border: "1px solid rgba(232,168,76,0.2)", borderRadius: 10, fontFamily: "var(--font-cinzel)", fontSize: 12, color: "rgba(232,168,76,0.6)", cursor: "pointer", letterSpacing: "0.04em" }}>{copied ? "✓ Link copiado" : "🔗 Copiar mi link"}</button>
          <p style={{ fontFamily: "var(--font-lato)", fontSize: 11, color: "rgba(240,234,214,0.25)", textAlign: "center", marginTop: 10, lineHeight: 1.4 }}>Tu posición sube cada vez que alguien entra por tu link. ¡El ranking se actualiza en tiempo real!</p>
        </div>
      )}
