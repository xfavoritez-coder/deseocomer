...
function ConcursoDetallePage() {
  ...
  const doJoin = async (refOverride?: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        } else if (errData.codigo === "TELEFONO_NO_VERIFICADO") {
  ...
  const rankingBlock = (
    <div style={{ background: "rgba(13,27,62,0.85)", border: "1px solid rgba(61,100,210,0.25)", borderRadius: 12, overflow: "hidden", position: "relative" }}>
          ...
          return (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderBottom: "1px solid rgba(61,100,210,0.1)", background: isMe ? "rgba(100,140,255,0.1)" : "transparent", border: isMe ? "1px solid rgba(100,140,255,0.2)" : undefined, borderRadius: isMe ? 8 : 0, position: "relative" }}>
              {!esSorteo ? (
                ...
                <div style={{ width: 22, height: 22, borderRadius: "50%", background: "rgba(232,168,76,0.08)", border: "1px solid rgba(232,168,76,0.15)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, flexShrink: 0 }}>🎲</div>
              )}
              {r.fotoUrl ? (
              ...
              {esSorteo && isAuthenticated && isParticipating && <span style={{ fontFamily: "var(--font-cinzel)", fontSize: 13, color: "#a7c4ff", whiteSpace: "nowrap" }}>{Math.max(1, r.referidos)} 🎟️ <span style={{ fontSize: 11, color: "rgba(167,196,255,0.6)" }}>~{probPct}%</span></span>}
              ...
              {tooltipActivo === "noparticipa_" + supportKey && <div style={{ position: "absolute", bottom: "calc(100% + 6px)", right: 14, background: "rgba(30,20,5,0.96)", border: "1px solid rgba(232,168,76,0.3)", borderRadius: 10, padding: "8px 14px", fontFamily: "var(--font-lato)", fontSize: 12, color: "rgba(240,234,214,0.7)", whiteSpace: "nowrap", zIndex: 10, lineHeight: 1.4 }}>Entra al concurso para apoyar 💛</div>}
              {tooltipActivo === "nologin_" + supportKey && <div style={{ position: "absolute", bottom: "calc(100% + 6px)", right: 14, background: "rgba(30,20,5,0.96)", border: "1px solid rgba(232,168,76,0.3)", borderRadius: 10, padding: "8px 14px", fontFamily: "var(--font-lato)", fontSize: 12, color: "rgba(240,234,214,0.7)", whiteSpace: "nowrap", zIndex: 10, lineHeight: 1.4 }}>Inicia sesión para apoyar 💛</div>}
            </div>
          );
        };
    ...
    <main style={{ background: "var(--bg-primary)", minHeight: "100vh" }}>
      ...
      {hasRefLink && !isAuthenticated && !refBannerDismissed && !authLoading && (() => {
        ...
        return (<>
          ...
          <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", width: "90%", maxWidth: 480, zIndex: 1000, background: "rgba(13,7,3,0.98)", border: "1px solid rgba(232,168,76,0.5)", borderRadius: 20, boxShadow: "0 0 60px rgba(0,0,0,0.8)", padding: "36px 28px", textAlign: "center" }}>
            ...
            <p style={{ fontFamily: "var(--font-lato)", fontSize: "1rem", color: "var(--text-primary)", lineHeight: 1.7, marginBottom: 24 }}><strong style={{ color: "var(--accent)" }}>{dn}</strong> participa en <strong style={{ color: "var(--accent)" }}>{c.premio}</strong> y necesita tu ayuda para sumar puntos. Regístrate gratis, súmale 3 puntos y <strong style={{ color: "var(--accent)" }}>tú también ganas 3 puntos</strong> en este concurso.</p>
            <Link href={`/registro?ref=${refUserId || refCodeRaw}&concurso=${concursoId}`} onClick={(e) => { const btn = e.currentTarget; btn.textContent = "Registrando..."; btn.style.opacity = "0.6"; btn.style.pointerEvents = "none"; }} style={{ display: "block", background: "var(--accent)", color: "var(--bg-primary)", fontFamily: "var(--font-cinzel)", fontSize: "0.85rem", fontWeight: 700, padding: "14px 28px", borderRadius: 14, textDecoration: "none", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "6px", textAlign: "center" }}>🎉 Registrarme gratis</Link>
            <p style={{ fontFamily: "var(--font-lato)", fontSize: "0.78rem", color: "rgba(240,234,214,0.4)", textAlign: "center", marginBottom: "10px" }}>💛 Ambos ganan 3 puntos automáticamente</p>
          </div>
        </>);
      })()}
      ...
      <div className="dc-cd-body">
        <div className="dc-cd-main" style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          {/* Sorteo — mecánica detallada (solo visible después de participar, después del countdown) */}
          {esSorteo && isParticipating && !isEnded && !isProgramado && (() => {
            ...
            return (
              <div style={{ background: "rgba(232,168,76,0.04)", border: "1px solid rgba(232,168,76,0.2)", borderRadius: 16, padding: "18px 20px" }}>
          ...
          <div style={{ background: "rgba(232,168,76,0.06)", border: "1px solid rgba(232,168,76,0.22)", borderRadius: 14, overflow: "hidden" }}>
            <div style={{ padding: 20 }}>
              <p style={{ fontFamily: "var(--font-cinzel)", fontSize: 14, color: "#e8a84c", textTransform: "uppercase", letterSpacing: "0.06em", textAlign: "center", fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>{isAuthenticated && isParticipating ? (esSorteo ? "🎲 ¡Ya estás en el sorteo!" : <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#e8a84c" strokeWidth="2"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>Tu link de participación</>) : esSorteo ? "🎲 Entra a este sorteo gratis" : "🏆 Participa en este concurso"}</p>
              <p style={{ fontFamily: "var(--font-lato)", fontSize: 14, color: "rgba(240,234,214,0.45)", textAlign: "center", marginTop: 6 }}>{isAuthenticated && isParticipating ? (esSorteo ? "Comparte tu link — cada amigo que entre por ti te da un boleto extra y más chances de ganar" : "Comparte este link y suma puntos para ganar") : esSorteo ? "Solo por entrar ya tienes un boleto. Cualquiera dentro puede ganar" : "Invita amigos y sube en el ranking para ganar"}</p>
              ...
              {isAuthenticated && isParticipating && refLink ? (
                <div style={{ marginTop: 14 }}>
                  ...
                  {/* Instagram story */}
                  <button onClick={() => { fetch(`/api/concursos/${concursoId}/track-share`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userId: user?.id, tipo: "instagram" }), keepalive: true }).catch(() => {}); window.open(`/story/${c?.slug || concursoId}/user?userId=${user?.id}`, "_blank"); }} style={{ width: "100%", marginTop: 8, background: "rgba(225,48,108,0.1)", border: "1px solid rgba(225,48,108,0.3)", borderRadius: 10, padding: 13, fontFamily: "var(--font-cinzel)", fontSize: "0.85rem", fontWeight: 700, color: "#E1306C", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, textTransform: "uppercase", letterSpacing: "0.06em" }}>
                ...
                <div style={{ marginTop: 16 }}>
                  <button onClick={handleJoin} disabled={joinLoading} style={{ display: "block", width: "100%", background: "#e8a84c", color: "#0a0812", fontFamily: "var(--font-cinzel)", fontSize: "0.85rem", fontWeight: 700, textTransform: "uppercase", padding: 14, borderRadius: 10, border: "none", cursor: joinLoading ? "wait" : "pointer", textAlign: "center", letterSpacing: "0.06em" }}>{joinLoading ? "Uniéndote..." : esSorteo ? "Entrar gratis al sorteo" : "🎉 Unirme al concurso"}</button>
                  {joinError && <p style={{ fontFamily: "var(--font-lato)", fontSize: "0.82rem", color: "#ff8c00", textAlign: "center", marginTop: 8 }}>{joinError}</p>}
                  <p style={{ fontFamily: "var(--font-lato)", fontSize: 13, color: "rgba(240,234,214,0.35)", textAlign: "center", marginTop: 8 }}>{esSorteo ? "Es gratis. Al cierre se elige un ganador al azar entre todos" : "Entra gratis y sube en el ranking para ganar"}</p>
                </div>
              ) : !isAuthenticated ? (
                <div style={{ marginTop: 16 }}>
                  {pendingRefName ? (
  ...
  { icon: "💛", pts: "+1", label: "Al recibir apoyo", id: "tt6", ttTitle: "Al recibir apoyo", ttText: "Cuando otro participante toca el corazón en tu perfil del ranking, tú ganas +1 punto. Solo puedes recibir un apoyo por persona cada 24 horas.", ttPts: "+1 punto para ti" },
      ...
      {showPhoneModal && user && (
        <VerificarTelefono
          ...
          onVerificado={() => {
            setShowPhoneModal(false);
            // Retry participation after phone verification
            doJoin();
          }}
      ...
      {recienUnido && refLink && (<>
        ...
        <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", zIndex: 500, width: "90%", maxWidth: 420, background: "rgba(20,12,35,0.98)", border: "1px solid rgba(232,168,76,0.3)", borderRadius: 20, padding: "32px 24px", textAlign: "center", boxShadow: "0 0 60px rgba(0,0,0,0.8)", animation: "dc-slideUp 0.4s ease" }}>
          ...
          {esSorteo ? (
            ...
            <>
              ...
              <div style={{ background: "rgba(61,184,158,0.08)", border: "1px solid rgba(61,184,158,0.2)", borderRadius: 12, padding: "14px 16px", marginBottom: 16 }}>
                <p style={{ fontFamily: "var(--font-lato)", fontSize: 14, color: "#3db89e", lineHeight: 1.6, margin: 0 }}>
                  Le sumaste <strong>+3 puntos</strong> a <strong>{refNameFromUrl || refNameRaw}</strong> y tú ganaste <strong>+3 puntos</strong>
                </p>
              </div>
              <p style={{ fontFamily: "var(--font-lato)", fontSize: 14, color: "rgba(240,234,214,0.6)", lineHeight: 1.6, marginBottom: 20 }}>Ahora invita a tus amigos para seguir subiendo en el ranking. Cada amigo que entre por tu link te da <strong style={{ color: "#e8a84c" }}>+3 puntos</strong> más.</p>
            </>
          ) : (
            <>
              <h3 style={{ fontFamily: "var(--font-cinzel)", fontSize: 18, fontWeight: 700, color: "#f5d080", textTransform: "uppercase", marginBottom: 8, letterSpacing: "0.04em" }}>¡Estás dentro!</h3>
          ...
          <button onClick={() => { fetch(`/api/concursos/${concursoId}/track-share`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userId: user?.id, tipo: "instagram" }), keepalive: true }).catch(() => {}); window.open(`/story/${c?.slug || concursoId}/user?userId=${user?.id}`, "_blank"); }} style={{ width: "100%", padding: 14, background: "rgba(225,48,108,0.1)", border: "1px solid rgba(225,48,108,0.3)", borderRadius: 12, fontFamily: "var(--font-cinzel)", fontSize: 15, fontWeight: 700, color: "#E1306C", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, marginBottom: 10, letterSpacing: "0.04em", textTransform: "uppercase" }}>
