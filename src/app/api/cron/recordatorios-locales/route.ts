  ...
  try {
    ...
    for (const local of locales) {
      ...
      if (dias >= 2 && !local.recordatorio2d) {
        ...
        dbUpdates.push(async () => { await prisma.local.update({ where: { id: local.id }, data: { recordatorio2d: true } }); });
        resultTypes.push("dia2");
      }
      ...
      if (dias >= 5 && !local.recordatorio5d) {
        ...
        dbUpdates.push(async () => { await prisma.local.update({ where: { id: local.id }, data: { recordatorio5d: true } }); });
    ...
    if (emailsToSend.length > 0) {
      ...
      for (let i = 0; i < emailsToSend.length; i += BATCH) {
        const batch = emailsToSend.slice(i, i + BATCH);
        const batchUpdates = dbUpdates.slice(i, i + BATCH);
        const batchTypes = resultTypes.slice(i, i + BATCH);
        try {
          await resend.batch.send(batch);
          // Mark as sent in DB
          await Promise.all(batchUpdates.map(fn => fn()));
          batchTypes.forEach(t => results[t]++);
        } catch (e) {
          console.error("[Cron recordatorio] batch error:", e);
