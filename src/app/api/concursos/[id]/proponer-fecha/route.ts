...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { localId, fecha } = await req.json();
    if (!localId || !fecha) return NextResponse.json({ error: "Faltan campos" }, { status: 400 });
    ...
    await resend.emails.send({
      ...
      subject: `📅 ${concurso.local.nombre} quiere coordinar la entrega de tu premio`,
...
<div style="text-align:center;margin-top:32px"><p style="color:#5a4028;font-size:12px">Hecho con 💛 y mucha hambre · DeseoComer.com</p></div>
    ...
    console.error("[Proponer fecha]", error);
