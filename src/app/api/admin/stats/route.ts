...
export async function GET(req: NextRequest) {
  ...
  try {
    const [
      ...
      localesManualesPendientes, localesReclamadosPendientes,
      ...
      prisma.local.count(),
      prisma.local.count({ where: { OR: [{ activo: true }, { estadoLocal: "ACTIVO", origenImportacion: "GOOGLE_PLACES" }] } }),
      // Locales registrados manualmente que no están activos (pendientes de aprobación)
      prisma.local.count({ where: { activo: false, origenImportacion: null } }),
      // Locales reclamados pero no aprobados
      prisma.local.count({ where: { reclamadoEn: { not: null }, activo: false } }),
